...
const customSchema = require('./models/custom-command');
...
client.on('message', async message => {
    ...
    if(message.mentions.users.first()) {
        if(message.mentions.users.first().id === '852204959373197323') return message.channel.send(`My prefix for \`${message.guild.name}\` is \`${p}\`, Type \`${p}help\` for help.`);
    }
    ...
    blacklist.findOne({ id : message.author.id}, async (err, data) => {
        ...
        if(!data) {
            ...
            const data = await customSchema.findOne({ Guild: message.guild.id, Command: cmd});
            if(data){
...
client.on('guildMemberAdd', async (member) => {
    welcomeSchema.findOne({ Guild: member.guild.id}, async (e, data) => {
        ...
        .setColor(`#8015EA`)
        .setDescription(`WELCOME, <@${user.id}> TO **${member.guild.name}** HOPE YOU HAVE A GREAT TIME HERE \n TOTAL MEMBERS: ${member.guild.memberCount}`)
        .setImage("https://media.giphy.com/media/xUPGGDNsLvqsBOhuU0/giphy.gif")
        .setTimestamp();
          const channel = member.guild.channels.cache.get(data.Channel);
          channel.send(`<@${user.id}>, Welcome to **${member.guild.name}**`)
          channel.send(embed);
    })
})
